...
ADSigTax <- separate(subset(TaxLDA,Group=="AOM"),1,into = c("K","P","C","O","F","G"),sep = "[.]")[,1:6]
...
TAX_Sig_Abun1 <- TAX_Sig_Abun[,-1] %>% apply(.,2,as.numeric) %>% 
  cbind(TAX_Sig_Abun[,1],.) %>% 
  group_by(Feature) %>%
  summarize_all(sum) 

plotdata <- pivot_longer(TAX_Sig_Abun1,
                         cols = 2:ncol(TAX_Sig_Abun1),
                         names_to = "SampleID",
...
plotdata <- plotdata %>% subset(.,Feature!="g__Listeria"&Feature!="g__Acetobacterium"&Feature!="f__Listeriaceae"&Feature!="f__Syntrophotaleaceae")
...
write.table(LEfSe_abundance,"LEfSe_abundance_lower.tsv",row.names = F,sep = "\t")
...
ggsave("LEfSe_sig_bar_ADlower.svg",p,width = 10,height = 8,units = "in")
